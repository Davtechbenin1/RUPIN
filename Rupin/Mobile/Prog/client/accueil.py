...
class accueil(box):
	...
	def initialisation(self):
		...
		self.where = self.user_dict.get('adresse',"Non définie")
		nom = self.user_dict.get('prénom') or self.user_dict.get('nom')

		self.menu = self.sc.manage_menu()

		self.accroch_text = f"""Bonjour {nom}, on mange quoi aujourd'hui ?"""
	...
	def Foreign_surf(self):
		...
		bo.add_icon_but(icon = "magnify",
			on_press = self.show_search,
			size_hint = (None,1),size = (dp(30),1))
		bo.add_button("Chercher un plat ou un restaurant",
			on_press = self.show_search,
		...
		b.add_icon_but(icon = "map-marker",
			...
			size_hint = (None,1),size = (dp(30),1))
		b.add_button(f"Livrer à {self.where}",
			on_press = self.show_where,
			text_color = self.sc.text_col3,
			halign = 'left')
		self.add_surf(b)
	...
	def add_one_menu_cat(self,cate,liste,*args):
		...
		for dic in liste:
			...
			bb = tt_f.add_button('',on_press = self.show_vent_surf,
				bg_color = None,info = dic.get('N°'))
			bb.categorie = cate
			bb.provenance = dic.get('prestataire',str())
			b.add_surf(tt_f)
...
# Gestion des actions
	...
	def show_where(self,wid):
		self.sc.add_refused_error("Non définie",text_color = self.sc.text_col1)
