	...
	try:
		...
		if con:
			...
			ws = websocket.WebSocketApp(
				...
				on_message=self.on_message,
				on_open=self.on_open)
			self.ws_client = self._ws_client(self,ws,'rupin')
...
def Connecte_to(self):
	...
	host = inf_dic.get('host') or "localhost"
	port = inf_dic.get('port') or 8010
	self.sc.th_entreprise = self.th_entreprise
	self.sc.session = self.session
	self.sc.th_url = self.th_url = f"http://{host}:{port}/api/"
	ret = self.sc.get_all_base()
	return True
