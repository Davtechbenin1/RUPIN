...
class Base_table:
	...
	from .Local import (TH_CACHE, set_error_mess, 
		on_message,on_open,local_backup,
		)
	...
	from .cloud import (get_sync_dir,save_msg_to_sync,
		...
		relance_conexion,send_to_cloud,msg_handler,
		_msg_handler)
	...
	def check_connexion_to_sever(self):
		self.th_entreprise = 'rupin' #self.Get_local("Info_gene")
	...
	def Save_image(self, path_img):
		...
		if path_img.lower().endswith((".png", ".jpg", ".jpeg")):
			try:
				...
				files = {"file": (f"{uuid.uuid4().hex}rupin.jpg", buffer, "image/jpeg")}
		...
		if response.status_code == 200:
			try:
				...
				if filename:
					...
					print("Réponse invalide du serveur (pas de nom de fichier)")
			except Exception as e:
...
# Gestion des commandes internes
	...
	def replace_this_(self, fichier):
		"""
		Renomme un fichier .rupin en .sent avec un timestamp en préfixe
		Exemple : 167901234567890_sent.rupin
		"""
		...
		sent_nom = nom_fichier.replace('.rupin', '.sent')
